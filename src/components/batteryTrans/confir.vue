    ...
    export default {
        ...
        methods: {
                ...
                this.$axios.get(url).then((data) => {
                    ...
                    if (res.code === 200) {
	                    if(res.data){
		                    ...
		                    console.log('电池id：' + this.battery.battery_id);
            ...
            sendData() {
                ...
                let that = this;
                let url = `addOrder?batteryId=${this.battery_id}&storageType=${storage_type}&account_id=${account_id}`
                this.$axios.post(url).then((data) => {
                    let res = data.data
            ...
            handleSubmit() {
                ...
                //提交这个订单/charge_factory/batterys/complete/{accountId}/{storage_type}
	            let order_number = this.order_number ? this.order_number : localStorage.getItem('exception_order_number')
                //let url = `completeScan?order_number=${order_number}&account_id=${this.account_id}&storage_type=${this.storageType}&battery_id=${this.battery_id}`
                let url = `completeScan?order_number=${order_number}&storage_type=${this.storageType}&battery_id=${this.battery_id}`
                this.$axios.post(url).then((data) => {
                    console.log(data);
                    let res = data.data
                    if (res.code === 200) {
		            ...
		            setTimeout( () => {
			            ...
			            if (res.code === 200) {
				            if(res.data != null){
					            ...
					            this.$router.push({path: `/batTra/abnFee1`})
				            }
