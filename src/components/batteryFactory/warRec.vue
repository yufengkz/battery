<template>
	<div class="war-rec">
		...
		<div class="ser">
			<router-link to="/batFac/warSch" tag="div">
				<yd-search fullpage placeholder="电池编号"></yd-search>
			</router-link>
			...
			<div id="dataList0">
				<ul v-for="(item,index) in newsList0" class="oul">
					...
					<li>
						...
						<a @click="orderDetail(item)"><span class="blue">查看详情 》</span></a>
					</li>
				</ul>
			</div>
		</div>
	...
	export default {
		...
		methods: {
			...
			getListDataFromNet(curNavIndex, pageNum, pageSize, successCallback, errorCallback) {
				...
				let storageType = ''
				if (curNavIndex == 0) storageType = '满电入库'
				if (curNavIndex == 1) storageType = '低电返厂'
				let obj = {
				...
				setTimeout(() => {
					...
					that.$axios.get(`charge_factory/storage_record`, {
						params: obj
					}).then((data) => {
						let res = data.data
						if (res.code === 200) {
