...
<script>
	...
	export default {
		...
		mounted() {
			...
			if (battery) this.battery = JSON.parse(battery)
			console.log(this.battery);
			this.battery_number = this.battery.battery_number
			this.battery_id = this.battery.battery_id || ''
			this.exception_order_number = localStorage.getItem('exception_order_number') || ''
						...
						setTimeout(() => {
							...
							this.$dialog.toast({
								...
								callback: () => {
									...
									let exception_order_number = res.data.exception_order_number

									localStorage.removeItem('exceptionsBattery')
									if (exception_order_number) {
										localStorage.setItem('exception_order_number', exception_order_number)
									} else {
